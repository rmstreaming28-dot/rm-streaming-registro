...
app.post('/api/perfiles',auth,async(req,res)=>{ const {nombre_perfil,nombre_cliente,telefono,plataforma,fecha_compra,fecha_vencimiento,meses_vendidos,precio}=req.body; try{ const r=await pool.query(`INSERT INTO perfiles (nombre_perfil,nombre_cliente,telefono,plataforma,fecha_compra,fecha_vencimiento,meses_vendidos,precio) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,[nombre_perfil,nombre_cliente,telefono,plataforma,fecha_compra||null,fecha_vencimiento||null,meses_vendidos||1,precio||0]); res.json(r.rows[0]); }catch(e){ res.status(500).json({error:String(e)}); }});
app.put('/api/perfiles/:id',auth,async(req,res)=>{ const {nombre_perfil,nombre_cliente,telefono,plataforma,fecha_compra,fecha_vencimiento,meses_vendidos,precio}=req.body; try{ const r=await pool.query(`UPDATE perfiles SET nombre_perfil=$1,nombre_cliente=$2,telefono=$3,plataforma=$4,fecha_compra=$5,fecha_vencimiento=$6,meses_vendidos=$7,precio=$8 WHERE id=$9 RETURNING *`,[nombre_perfil,nombre_cliente,telefono,plataforma,fecha_compra||null,fecha_vencimiento||null,meses_vendidos||1,precio||0,req.params.id]); res.json(r.rows[0]); }catch(e){ res.status(500).json({error:String(e)}); }});
