...
// PERFILES (flat)
app.get('/api/perfiles',auth,async(req,res)=>{ const q=req.query.q||''; try{ const r=await pool.query(`SELECT * FROM perfiles WHERE nombre_cliente ILIKE $1 OR telefono ILIKE $1 OR correo ILIKE $1 ORDER BY fecha_vencimiento ASC`,[`%${q}%`]); res.json(r.rows); }catch(e){ res.status(500).json({error:String(e)}); }});
app.post('/api/perfiles',auth,async(req,res)=>{ const {nombre_cliente,telefono,correo,plataforma,fecha_compra,fecha_vencimiento,meses_vendidos,precio,proveedor}=req.body; try{ const r=await pool.query(`INSERT INTO perfiles (nombre_cliente,telefono,correo,plataforma,fecha_compra,fecha_vencimiento,meses_vendidos,precio,proveedor) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,[nombre_cliente,telefono,correo,plataforma,fecha_compra||null,fecha_vencimiento||null,meses_vendidos||1,precio||0,proveedor]); res.json(r.rows[0]); }catch(e){ res.status(500).json({error:String(e)}); }});
app.put('/api/perfiles/:id',auth,async(req,res)=>{ const {nombre_cliente,telefono,correo,plataforma,fecha_compra,fecha_vencimiento,meses_vendidos,precio,proveedor}=req.body; try{ const r=await pool.query(`UPDATE perfiles SET nombre_cliente=$1,telefono=$2,correo=$3,plataforma=$4,fecha_compra=$5,fecha_vencimiento=$6,meses_vendidos=$7,precio=$8,proveedor=$9 WHERE id=$10 RETURNING *`,[nombre_cliente,telefono,correo,plataforma,fecha_compra||null,fecha_vencimiento||null,meses_vendidos||1,precio||0,proveedor,req.params.id]); res.json(r.rows[0]); }catch(e){ res.status(500).json({error:String(e)}); }});
